...
Retrieves the **order book snapshot** (Market Depth / DOM) for a specified symbol from MT5.<br/>
Requires calling `subscribe_market_book()` beforehand to enable the data source.<br/>
Returns bid/ask price levels and corresponding volumes, which can be used for order flow analysis or liquidity monitoring.
