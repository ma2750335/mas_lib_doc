...
Subscribes to **market depth data (DOM - Depth of Market)** for a specified symbol.<br/>
After a successful subscription, `get_market_book()` can be called at any time to retrieve the current order book snapshot.<br/>
When no longer needed, call `unsubscribe_market_book()` to release the subscription resources.
