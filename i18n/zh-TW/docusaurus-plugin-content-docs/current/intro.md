...
# <img src="/img/mas_logo.png" alt="brain icon" width="48" style={{ verticalAlign: 'middle' }} /> MasQuant Python SDK 簡介

MasQuant Python SDK 是一套專為外匯與多資產策略交易者設計的輕量級開源函式庫。  
它支援**自動化交易、量化策略回測**與**MT5 部署整合**，讓你**無需深入理解券商 API 或通訊封包**，即可快速開發與執行交易策略。

---

## 🚀 MasQuant SDK 能幫你完成哪些任務？

- 📥 取得即時行情與歷史 K 線數據（含外匯、指數等）
- 📈 執行策略自動下單、模擬交易與真實交易績效對比
- 🧠 自定義交易邏輯（支援停利/停損、技術指標如 RSI、MACD 等）
- 🔗 整合 MasQuant Web 平台：帳戶資訊、績效報表一站呈現

---

## 📊 策略回測與績效報表功能

MasQuant SDK 可於回測與實單交易後，自動生成多種專業級報表，助你評估策略品質與風險控制指標：
...
☑️ 所有報表均可在 MasQuant SDK查看與下載，或透過 SDK 匯出（視方案等級而定）

---

## 🧩 使用 MasQuant SDK 前的準備事項

1. 註冊 MasQuant 平台帳號[【立即註冊↗】](https://mas.mindaismart.com/authentication/sign-up)（免費）
2. 下載 MasQuant 軟體[【點我進行下載↗】](https://mindaismart.com/product_lib)，登入後可查看目前方案等級
3. 安裝 MasQuant SDK 並啟動第一個策略專案

---

## ⚙️ 安裝 MasQuant SDK 並快速開始
...
| API 使用手冊 | 所有 MasQuant 函式、參數與回傳結果一覽 |
...
只需輸入你的交易邏輯描述，MasQuant AI 策略助理可協助你：[【了解更多 ↗】](https://mindaismart.com/product_ai)
...
```diff
🖋 步驟 1：在MasQuant AI 策略助理畫面，輸入策略邏輯
...
👉 現在就體驗 MasQuant AI 策略助理，讓量化交易更輕鬆、更快速！
```
